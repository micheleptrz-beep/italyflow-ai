...
def _hero(db, user_id: int, page: str = "dashboard"):
    if not _VISUALS_OK:
        return None
    try:
...
def check(body: CheckIn, request: Request, db: Session = Depends(get_db)):
    ...
    text = body.text or ""
    if body.layers:
        text = " ".join(str(l.get("text", "")) for l in body.layers if l.get("type") == "text")
    text = text.lower()
    keywords = ("ingredients", "allergens", "net weight", "best before", "manufacturer", "origin")
