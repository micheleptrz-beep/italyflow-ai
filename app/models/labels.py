...
NOTE: no ForeignKey to legacy 'users' table (not registered in SQLAlchemy metadata).
      user_id and author_user_id are plain integer columns.
"""
...
class IfLabel(Base):
    ...
    versions = relationship("IfLabelVersion", back_populates="label",
                            ...
                            foreign_keys="IfLabelVersion.label_id")
...
class IfLabelVersion(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    label_id = Column(Integer, ForeignKey("if_labels.id", ondelete="CASCADE"),
                      nullable=False, index=True)
    version_no = Column(Integer, nullable=False)
    author_user_id = Column(Integer, nullable=False)
    layers = Column(JSON, default=list)
    compliance_snapshot = Column(JSON, default=dict)
    note = Column(String(500), nullable=True)
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))

    label = relationship("IfLabel", back_populates="versions",
